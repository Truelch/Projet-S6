...
Turret::Turret(float x, float y, float rotation, const char * filename, Game * game, Layer * layer, 
				float x_relative, float y_relative, float missile_speed, int damage, float cooldown, float range_max, Unit * unit): Displayable(x,y,rotation,filename,game,layer), _unit(unit)
{
